...
def test_create_achievement(test_client):
    data = {
    ...
    response = test_client.post('/api/achievements', json=data)
...
def test_update_achievement(test_client):
    # Assuming there's an achievement with ID '1' in the test database
    achievement_id = '1'
    data = {
    ...
    response = test_client.put(f'/api/achievements/{achievement_id}', json=data)
    assert response.status_code in [200, 404]
    data = json.loads(response.data)
    assert 'message' in data
    if response.status_code == 200:
